    ...
    public Parameter[] getParameters(InvokeTarget target) {
        ...
        // /////////Resolve parameter names by asm
        {
            if (log.isDebugEnabled()) {
                log.debug("Analyzing method {}.{}", controllerClass.getName(),
                        method.getName());
            }
            ...
            for (MethodNode m : (List<MethodNode>) node.methods) {
                if (methodName.equals(m.name)) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("Method {}.{} has parameter: {}",
            ...
            if (!methodNodeGot) {
                throw new IllegalStateException("Can't get method "
                        + method.getName() + " from ASM");
            }
            return parameters;
        }
    }
