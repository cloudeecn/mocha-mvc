...
import works.cirno.mocha.ComparableClassWrapper;
...
public class ComparableClassWrapperTest {
    ...
    public void ensureCompare(ComparableClassWrapper aw, ComparableClassWrapper bw, ComparableClassWrapper cw) {
        ComparableClassWrapper t;
        if (aw.compareTo(bw) > 0) {
            t = aw;
            aw = bw;
            bw = t;
        }
        if (bw.compareTo(cw) > 0) {
            t = bw;
            bw = cw;
            cw = t;
        }
        ...
        if (aw.compareTo(bw) < 0 && bw.compareTo(cw) < 0 && aw.compareTo(cw) < 0 &&
                bw.compareTo(aw) > 0 && cw.compareTo(bw) > 0 && cw.compareTo(aw) > 0) {
            if (cw.getContent().isAssignableFrom(bw.getContent()) || bw.getContent().isAssignableFrom(cw.getContent())) {
                Assert.assertEquals(bw.getContent().getName() + " <= " + cw.getContent().getName(), cw.getContent().isAssignableFrom(bw.getContent()), bw.compareTo(cw) <= 0);
            }

            if (cw.getContent().isAssignableFrom(aw.getContent()) || aw.getContent().isAssignableFrom(cw.getContent())) {
                Assert.assertEquals(aw.getContent().getName() + " <= " + cw.getContent().getName(), cw.getContent().isAssignableFrom(aw.getContent()), aw.compareTo(cw) <= 0);
            }

            if (aw.getContent().isAssignableFrom(bw.getContent()) || bw.getContent().isAssignableFrom(aw.getContent())) {
                Assert.assertEquals(bw.getContent().getName() + " <= " + aw.getContent().getName(), aw.getContent().isAssignableFrom(bw.getContent()), bw.compareTo(aw) <= 0);
            }
        } else {
            throw new IllegalStateException("Compare result for " + aw + " " + bw + " " + cw + " is wrong (" + aw.compareTo(bw) + ", " + bw.compareTo(cw) + ", " + aw.compareTo(cw) + ", " + bw.compareTo(aw) + ", " + cw.compareTo(bw) + ", " + cw.compareTo(aw) + ")");
        }
    }
                ...
                if (i != j) {
                    for (int k = 0; k < max; k++) {
                        if (i != k && j != k) {
                            ...
                            Class<?> b = classes[j];
                            Class<?> c = classes[k];
                            ensureCompare(new ComparableClassWrapper(a), new ComparableClassWrapper(b), new ComparableClassWrapper(c));
                        }
                    }
                }
            }
